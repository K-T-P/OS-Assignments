...
int main()
{
    ...
    int num_of_processes = (rows-w_rows+1)*(cols-w_cols+1);                                  // number of processes, usually equals number of cores, which is in most cases 8.
   ...
   // int row_per_proc = NULL / NULL;                               // the number of rows in result matrix that every process take care of. Use variables defined above.
    ...
    while (num_of_processes)                                                // condition on when to finish, which means when the result matrix is ready.
    {
        num_of_processes--;
        int temp=vfork();
        if(temp==0){
            int rowStartIndex=num_of_processes/result_rows;
            int columnStartIndex=num_of_processes%result_cols;
            int sum=0;
            for(int i=0;i<w_rows;i++){
                for(int j=0;j<w_cols;j++){
                    sum+=(mat[i+rowStartIndex][j+columnStartIndex]*window[i][j]);
                }
            }
            result[rowStartIndex][columnStartIndex]=sum;
            exit(0);
        }
    }
    // end section
